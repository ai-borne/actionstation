...
- [ ] B13 **ActionStation checkout is broken: Razorpay rejects our test API key (found 2026-09-20 during the drill).** `createRazorpayOrder` fails with `BAD_REQUEST_ERROR: Authentication failed` (Cloud Logging 2026-09-20 06:02:22 UTC, revision `createrazorpayorder-00012`). Secret Manager `RAZORPAY_KEY_ID` v1 = v2 = `rzp_test_SWt9omzPQJdCRT` (March). The Razorpay dashboard (test mode) lists a single active key, `rzp_test_TRFCZQwk1XNqur`, **generated 18 Aug 2026**; regenerating a key retires the old one, and 18 Aug is the day SSBMax's first test payments appear, so the shared account's key was regenerated for SSBMax. Upgrading has therefore failed for everyone since 18 Aug, and it never showed because nothing alerted on it (the new `webhook_processing_error` alert now does). Fix `(You)`: put the *current* test key id and secret into Secret Manager (`RAZORPAY_KEY_ID`, `RAZORPAY_KEY_SECRET`; the secret is only shown at generation, so take it from SSBMax's config) and redeploy the three functions that pin them (Runbook 5). **Never click Regenerate Key** — that would break SSBMax. This is the concrete case for B12: separate accounts, or accept that one product's key change breaks the other
