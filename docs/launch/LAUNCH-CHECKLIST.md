...
| Payments | Razorpay in **LIVE mode** for ActionStation since 2026-09-24 (`rzp_live_`, Secret Manager v4; B3, B24 proven). SSBMax on the same account still uses **test keys** (owner, 2026-09-25). Earlier test-mode history follows. Stripe secret is a **placeholder** (not launching). **Proven end to end 2026-09-20 (Sprint 2b)**: order → payment → webhook 200 → Pro → refund → Free → delete-with-plan (B2, B4, B6, B7 ticked). Functions pin secret v3 (`razorpaywebhook-00013`, `createrazorpayorder-00013`, `onuserdeleted-00007`); v1/v2 disabled. Open: B16 (late `payment.captured` retry after a refund re-grants Pro), B12 (shared account), B1/B3/B5/B9 (yours) |
...
- [x] B5 Real small live payment + refund drill — *Done as B24 (2026-09-24, ₹1 pay, webhook 200, refund, downgrade). Not yet observed: SSBMax's live-side filter log, because SSBMax is still on test keys and has no live traffic (see B12 update)*
...
- [ ] B12 **Razorpay account shared with SSBMax.** Decided 2026-09-25: one account. ActionStation LIVE; SSBMax on TEST keys and its Razorpay integration is being RETIRED (store billing via RevenueCat, which does not support Razorpay), so it never goes live. Open: retirement R1-R11 in `docs/payments/PAYMENT-STATE.md`; live webhook keeps 7 events deliberately. See `docs/payments/PAYMENT-STATE.md` `(You)` → [evidence](LAUNCH-EVIDENCE.md#b12)
...
- [ ] G18 Upload the logo where only a console can: Google OAuth consent screen (square PNG, 120×120 px, ≤1 MB; export from `public/pwa-512x512.png`; adding it can trigger Google's branding re-review, so do it with A10b), Razorpay checkout/business logo, and any Workspace/social avatars `(You)` — *Checked 2026-09-25 (read-only): Google consent screen Branding shows verified branding but an EMPTY App logo slot (Remove disabled) — the 2026-09-20 upload is not there; Razorpay checkout logo not located in the dashboard. Logo to use: the blue circle-check mark (`public/pwa-512x512.png`). Changing the Google logo may trigger a branding re-review, so it waits for the owner's go-ahead*
