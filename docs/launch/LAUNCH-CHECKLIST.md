...
- [x] C19 **PR preview deploys fail: Hosting preview-channel quota full (found 2026-09-25).** `(Claude)` — *`preview.yml` made `pr-<N>` per PR (7-day expiry) and never deleted it; 50 channels (`pr-51`…`pr-101`, all closed PRs) hit the 50-per-site cap and PR #102 got 429 "channel quota reached". Unblocked 2026-09-25 by deleting those 50 channels (only `live` left). Fix: `cleanup` job deletes the channel on PR close, expiry cut to 3d; guarded by `previewChannelCleanup.structural.test.ts`. First live runs (#102/#104 close, 2026-09-25) failed: the job had no checkout, so `hosting:channel:list` exited 1 "Not in a Firebase app directory"; fixed by adding `actions/checkout` (structural test). **Proven live 2026-09-25:** #105 merged (`b1ccbc1`); its close run 36138107433 `Delete Preview Channel` succeeded and logged "Successfully deleted channel pr-105"; `hosting:channel:list` afterwards shows no `pr-105`. The close event runs the merged PR's own workflow version (not `main`'s), which is why #104's own close already ran the job*
...
- [ ] F1 500+ node workspace: performance, spatial chunking, progressive loading — *2026-09-26, `e2e/specs/performance.spec.ts` (500 seeded cards, Vite dev server, local): zoom/pan 58-60 fps, worst frame 17-50 ms; **open takes 3.8-4.6 s locally and 17-23 s on the CI runner, against the 3 s target** (dev build, so unproven either way). Open: measure a production build, and 1,000 cards*
...
- [ ] F5 Touch on canvas: pinch zoom, drag nodes — *2026-09-26, `e2e/specs/touch.spec.ts` (emulated touch, local, 3/3): two-finger pinch zooms, one-finger drag on empty canvas pans, one-finger drag moves a card; no bug found. Open: a real iPhone and Android phone `(You)`*
- [ ] F6 Accessibility: Lighthouse a11y 90+ (target 95+), keyboard-only flows, VoiceOver and NVDA passes — *2026-09-26 (local, dev server): Lighthouse a11y was landing 0.94 / login 0.98; now 1.00 / 1.00 after fixing (a) the skip link, which pointed at `#main-canvas` on pages that have none (now only rendered in the signed-in app) and (b) landing CTA text at contrast 2.57, because the global `a { color: inherit }` beats Tailwind utilities (colour now inline). Signed-in canvas: axe (wcag2a/aa + best-practice) found a missing `<h1>`, added; now 0 violations (`accessibility.spec.ts`). Keyboard: skip link reachable and moves focus to the canvas; a card is created and typed with keyboard alone. Open: Lighthouse on the production build, VoiceOver and NVDA `(You)`. Observed: in Playwright the first Tab lands on the canvas radar, not the skip link; not reproduced or ruled out in a real browser*
- [x] F7 **Autosave writes only changed nodes/edges in production (#102 deployed 2026-09-25).** `(You + Claude)` — *Not proven. Cloud Monitoring per-minute counts for one editing session (20:32–20:38 IST) showed 143 reads / 174 writes in two bursts with 3–5 writes in the quiet minutes: consistent with delta saves, not conclusive (unknown edit timing; tab may have run the old build). Tick after a scripted session: hard-reload (one full-sync burst), then one edit per 2 minutes; expect 1–2 writes and about 0 reads per edit. **Verified live 2026-09-25 (after #109/#110 deployed):** Code Hacks (7 nodes): the session's first full sync wrote all 7 at 21:26:42; three later saves at 21:30:59, 21:31:16 and 21:31:26 each wrote exactly ONE node doc (4 nodes kept 21:26:42; the old code would have rewritten all 7 every save). Read the timestamps, not counts. Not measured: reads per save (Monitoring lag) and edge writes (edges have no `updatedAt`); three docs changed for two edited cards, cause unknown (drag/resize/blur commit)*
- [x] F8 **Node `updatedAt` was stored as a map, not a timestamp (found 2026-09-25).** `(Claude, TDD)` — *154 of 170 prod node docs held `{_methodName: "serverTimestamp"}`: `removeUndefined` copied the Firestore sentinel (and `Timestamp`/`GeoPoint`) into a plain object. The app falls back to "now" for such values, which skews `mergeNodes` last-writer-wins and search recency. Fix: `removeUndefined` recurses into plain objects only (`firebaseUtils.ts`). Tick after deploy + one session start (its full sync rewrites every node of that workspace): a `runQuery` over `nodes` shows `timestampValue` for the opened workspaces. **Verified live 2026-09-25 21:26 IST:** Code Hacks' 7 nodes now store `updatedAt` as a real timestamp (server-wide 23 timestamps, was 16); the other 147 docs stay maps until their workspace next saves*
- [x] F9 **Workspaces with pre-March docs could never save ("Save failed", 403 permission-denied; found 2026-09-25).** `(Claude, TDD)` → [evidence](LAUNCH-EVIDENCE.md#f9) — *Docs written before 2026-03-15 have no `userId`; the rules' `!resource.data.userId` errors on a missing field, so every update was denied and a save touching one failed atomically (4 workspaces: 24 nodes, 5 edges). Fix: `|| !('userId' in resource.data)` in the three guards; not caused by #102/#109. Tick after the rules deploy: reopen the workspace, save succeeds, and no node/edge doc lacks `userId` in the opened workspaces. **Verified live 2026-09-25:** rules deployed 21:21 IST; the owner's tab saved at 21:26; Code Hacks' 7 nodes and 2 edges now carry `userId`. Remaining docs without it (17 nodes, 3 edges) sit under workspace ids that have no workspace doc (deleted workspaces, cannot be opened, so they never save)*
- [x] F10 **Sidebar node counts were wrong: a save that finished after a workspace switch wrote its count onto the other workspace (found 2026-09-25).** `(Claude, TDD)` → [evidence](LAUNCH-EVIDENCE.md#f10) — *`useSaveCallback.save()` read the current-workspace ref after its network calls. Fix: capture the workspace at save start (test written first, seen failing with `{id: ws-B, nodeCount: 3}`). Explains Dan Koe Ideas 49 (7 docs), Human 3.0 0 (49 docs), Code Hacks 29 (7 docs, matches the canvas); content is in the right workspaces. Existing counts heal when each workspace is next opened and saved. Tick after deploy + opening those workspaces: stored `nodeCount` equals the actual docs. **Follow-up 2026-09-25 (owner: "corrects while open, reverts on hard reload"):** #111 was not enough. Server still held Code Hacks 49 and Dan Koe Ideas 49 (real 7) and SSBMax 2 (real 0). Cause 2: opening a workspace sets its count in memory (`useWorkspaceSwitcher`), and the save 2 s later compared against that value, saw "unchanged" and never wrote it (only the first workspace of a tab session got written). Fix: `useSaveCallback` remembers the count it wrote per workspace and writes once per opened workspace. Earlier claim "opening heals counts" was wrong before this fix. **Verified live 2026-09-25 23:19 IST (#111, #112 deployed):** after the owner opened the affected workspaces on the new build, every real workspace's stored `nodeCount` equals its actual node docs (13 of 13; Dan Koe Ideas 49 -> 7, Code Hacks 49 -> 7, Human 3.0 49, SSBMax 2 -> 0)*
- [x] F11 **Orphaned node/edge docs from deleted workspaces (found 2026-09-25).** `(Claude, owner approved)` → [evidence](LAUNCH-EVIDENCE.md#f10) — *The delete cascade only shipped 2026-03-15, so earlier deletions left their cards behind. Owner's orphans: 35 nodes + 3 edges in 5 workspace ids, no Storage refs; 2 more orphan groups belong to OTHER accounts and are not ours to touch. **Done 2026-09-25 21:43 IST:** owner chose to delete all of them (incl. two dated notes); 35 nodes + 3 edges removed in one atomic commit (each with an exists-precondition). Real workspaces unchanged (129 nodes, 67 edges), other accounts' orphans untouched. Recoverable from the 2026-09-25 export (`…-firestore-backups-immutable/2026-09-25/`, 30-day retention)*
- [x] F12 **Switch-away snapshots pile up in localStorage and can be replayed over newer data (found 2026-09-25).** `(Claude, TDD)` → [evidence](LAUNCH-EVIDENCE.md#f12) — *Every workspace switch queued a full snapshot of the workspace being left; nothing removed it after a good online save. The owner's browser held 18 (630 KB), 5 of them 5.6 days old. Fix (owner chose "save on leave, queue only offline"): online leave starts a save and queues only if it reports failure; a successful save discards older snapshots; `deleteWorkspace` discards its snapshot; a drain drops never-failed snapshots of workspaces that no longer exist. Snapshots that already failed to sync are never auto-removed (maybe the only copy). **Owner decided to drop the 5 old snapshots (31 cards + 22 edges for 3 workspaces missing on the server, SSBMax's 2 cards, and the 2 notes deleted from the server on 2026-09-25); removed from the browser queue 2026-09-25 22:4x IST (18 -> 13 snapshots, 630 -> 510 KB; selection guarded by exact workspace ids, the 13 recent ones untouched).** #113 deployed (`e8b1c88`). **Verified live 2026-09-25 23:17 IST (#113 deployed 22:3x, #112 23:1x):** after the owner opened 8 workspaces on the new build the browser queue went 13 -> 5 snapshots (510 -> 170 KB): each opened workspace's snapshot was discarded by its successful save, none was added by the switching (newest remaining snapshot is 83 min old, from before the session), none has retryCount > 0. The 5 left: 4 for workspaces not opened yet and 1 (6.5 h old, never failed) for a workspace missing from the list, which the next drain prunes*
- [x] F13 **Background Sync was registered but nothing consumed it (found 2026-09-25; #114, #116).** `(Claude, TDD)` — *No service worker sync handler exists and the queue lives in localStorage, so the registered tag was never consumed. The indicator stuck on "Syncing in background..." and `useQueueDrainer` skipped its manual drain for Pro users when a tag was registered, so the queue never flushed on reconnect. Fix: registration, `backgroundSyncService` and the pricing copy removed; the queue always drains on reconnect. Guard: `noOrphanBackgroundSync.structural.test.ts`. Not yet re-checked on the live site.*
