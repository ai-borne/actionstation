...
- [ ] F11 **Orphaned node/edge docs from deleted workspaces (found 2026-09-25).** `(You decide, Claude executes)` → [evidence](LAUNCH-EVIDENCE.md#f10) — *The delete cascade only shipped 2026-03-15, so earlier deletions left their cards behind. Owner's orphans: 35 nodes + 3 edges in 5 workspace ids, no Storage refs; 2 more orphan groups belong to OTHER accounts and are not ours to touch. Not deleted yet: waiting for the owner's decision (one group holds two dated notes that may be real). Backups exist (daily export, latest 2026-09-25)*
