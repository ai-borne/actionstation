...
- [ ] F9 **Workspaces with pre-March docs could never save ("Save failed", 403 permission-denied; found 2026-09-25).** `(Claude, TDD)` → [evidence](LAUNCH-EVIDENCE.md#f9) — *Docs written before 2026-03-15 have no `userId`; the rules' `!resource.data.userId` errors on a missing field, so every update was denied and a save touching one failed atomically (4 workspaces: 23 nodes, 5 edges). Fix: `|| !('userId' in resource.data)` in the three guards; not caused by #102/#109. Tick after the rules deploy: reopen the workspace, save succeeds, and no node/edge doc lacks `userId` in the opened workspaces*
